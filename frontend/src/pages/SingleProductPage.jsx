...
export const SingleProductPage = () => {
  ...
  useEffect(() => {
    if (singleProduct && singleProduct.allergies) {
      const matchingAllergies = singleProduct.allergies.filter(allergy => userAllergy.includes(allergy));
        ...
        <div className="w-full tablet:w-11/12 tablet:m-auto tablet:flex pb-12 desktop:w-9/12 relative">
          ...
          <div className="w-9/12 m-auto tablet:m-0 py-6 text-text-light font-heading tablet:pl-8 desktop:pl-16">
            ...
            <div className="flex gap-6">
              <div className="flex">
                <button onClick={handleDecrement} className="bg-text-light text-text-dark w-5 rounded-l-xl">-</button>
                <span className="bg-text-light text-text-dark w-12 flex items-center justify-center text-sm">{quantity}</span>
                <button onClick={handleIncrement} className="bg-text-light text-text-dark w-5 rounded-r-xl">+</button>
              </div>
              <button className="w-24 h-6 text-xs bg-strong-yellow p-1 rounded-full text-text-dark hover:bg-main-yellow">{addToCart}</button>
            </div>
          </div>
        </div>
      )}
